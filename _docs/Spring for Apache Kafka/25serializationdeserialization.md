---
title: Serialization, Deserialization, and Message Conversion
category: Spring for Apache Kafka
order: 26
permalink: /Spring for Apache Kafka/serdes/
description: 메시지 직렬화, 역직렬화, 변환 설정 방법
image: ./../../images/spring/logo.png
lastmod: 2025-09-12T23:34:00+09:00
comments: true
originalRefName: 스프링 카프카
originalRefLink: https://docs.spring.io/spring-kafka/reference/kafka/serdes.html
parent: Reference
...
- [JSON](#json)
  ...
  + [Using Methods to Determine Types](#using-methods-to-determine-types)
  + [Programmatic Construction](#programmatic-construction)
- [Delegating Serializer and Deserializer](#delegating-serializer-and-deserializer)
  + [Using Headers](#using-headers)
  + [By Type](#by-type)
  + [By Topic](#by-topic)
- [Retrying Deserializer](#retrying-deserializer)
- [Spring Messaging Message Conversion](#spring-messaging-message-conversion)
  + [Using Spring Data Projection Interfaces](#using-spring-data-projection-interfaces)
- [Using ErrorHandlingDeserializer](#using-errorhandlingdeserializer)
- [Payload Conversion with Batch Listeners](#payload-conversion-with-batch-listeners)
- [ConversionService Customization](#conversionservice-customization)
- [Adding Custom HandlerMethodArgumentResolver to @KafkaListener](#adding-custom-handlermethodargumentresolver-to-kafkalistener)
...
- `JacksonJsonSerializer.TYPE_MAPPINGS` (default `empty`): See [Mapping Types](https://docs.spring.io/spring-kafka/reference/kafka/serdes.html#serdes-mapping-types).
...
- `JacksonJsonDeserializer.TRUSTED_PACKAGES` (default `java.util`, `java.lang`): Comma-delimited list of package patterns allowed for deserialization. `*` means deserializing all.
- `JacksonJsonDeserializer.TYPE_MAPPINGS` (default `empty`): See [Mapping Types](https://docs.spring.io/spring-kafka/reference/kafka/serdes.html#serdes-mapping-types).
- `JacksonJsonDeserializer.KEY_TYPE_METHOD` (default `empty`): See [Using Methods to Determine Types](https://docs.spring.io/spring-kafka/reference/kafka/serdes.html#serdes-type-methods).
- `JacksonJsonDeserializer.VALUE_TYPE_METHOD` (default `empty`): See [Using Methods to Determine Types](https://docs.spring.io/spring-kafka/reference/kafka/serdes.html#serdes-type-methods).
...
If you use [Spring Boot](https://docs.spring.io/spring-boot/3.5/reference/messaging/kafka.html), you can provide these properties in the `application.properties` (or yaml) file. The following example shows how to do so:
...
> When using Spring Boot and overriding the `ConsumerFactory` and `ProducerFactory` as shown above, wild card generic types need to be used with the bean method return type. If concrete generic types are provided instead, then Spring Boot will ignore these beans and still use the default ones.
...
To provide type mapping programmatically, similar to [Using Methods to Determine Types](https://docs.spring.io/spring-kafka/reference/kafka/serdes.html#serdes-type-methods), use the `typeFunction` property.
...
Alternatively, as long as you don’t use the fluent API to configure properties, or set them using `set*()` methods, the factories will configure the serializer/deserializer using the configuration properties; see [Configuration Properties](https://docs.spring.io/spring-kafka/reference/kafka/serdes.html#serdes-json-config).
...
For another technique to send different types to different topics, see [Using `RoutingKafkaTemplate`](https://docs.spring.io/spring-kafka/reference/kafka/sending-messages.html#routing-template).
...
Starting with version 2.8.3, you can configure the serializer to check if the map key is assignable from the target object, useful when a delegate serializer can serialize sub classes. In this case, if there are ambiguous matches, an ordered `Map`, such as a `LinkedHashMap` should be provided.
...
> On the consumer side, you can configure a `JacksonJsonMessageConverter`; it can handle `ConsumerRecord` values of type `byte[]`, `Bytes` and `String` so should be used in conjunction with a `ByteArrayDeserializer`, `BytesDeserializer` or `StringDeserializer`. (`byte[]` and `Bytes` are more efficient because they avoid an unnecessary `byte[]` to `String` conversion). You can also configure the specific subclass of `JacksonJsonMessageConverter` corresponding to the deserializer, if you so wish.On the producer side, when you use Spring Integration or the `KafkaTemplate.send(Message<?> message)` method (see [Using `KafkaTemplate`](https://docs.spring.io/spring-kafka/reference/kafka/sending-messages.html#kafka-template)), you must configure a message converter that is compatible with the configured Kafka `Serializer`.`StringJacksonJsonMessageConverter` with `StringSerializer``BytesJacksonJsonMessageConverter` with `BytesSerializer``ByteArrayJacksonJsonMessageConverter` with `ByteArraySerializer`Again, using `byte[]` or `Bytes` is more efficient because they avoid a `String` to `byte[]` conversion.For convenience, starting with version 2.3, the framework also provides a `StringOrBytesSerializer` which can serialize all three value types so it can be used with any of the message converters.
...
To enable this feature, use a `JacksonProjectingMessageConverter` configured with an appropriate delegate converter (used for outbound conversion and converting non-projection interfaces). You must also add `spring-data:spring-data-commons` and `com.jayway.jsonpath:json-path` to the classpath.
...
Alternatively, you can configure the `ErrorHandlingDeserializer` to create a custom value by providing a `failedDeserializationFunction`, which is a `Function<FailedDeserializationInfo, T>`. This function is invoked to create an instance of `T`, which is passed to the listener in the usual fashion. An object of type `FailedDeserializationInfo`, which contains all the contextual information is provided to the function. You can find the `DeserializationException` (as a serialized Java object) in headers. See the [Javadoc](https://docs.spring.io/spring-kafka/docs/4.0.0/api/org/springframework/kafka/support/serializer/ErrorHandlingDeserializer.html) for the `ErrorHandlingDeserializer` for more information.
...
See also [Null Payloads and Log Compaction of `Tombstone` Records](https://docs.spring.io/spring-kafka/reference/kafka/tombstones.html).