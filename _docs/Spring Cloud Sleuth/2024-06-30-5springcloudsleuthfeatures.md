...
lastmod: 2024-06-25T13:30:00+09:00
comments: true
...
기본으로 제공하는 메커니즘을 사용하려면 `spring.sleuth.propagation.type` 프로퍼티를 이용하면 된다. 이 값을 목록으로 지정하면 더 많은 트레이싱<sup>propagation</sup> 헤더를 전파할 수 있다.
...
분산 트레이싱<sup>distributed tracing</sup>은 필드들을 내부로 전파하고, 여러 서비스 간에 전파하는 방식으로 트레이스<sup>trace</sup>를 서로 연결시킴으로써 동작한다. 전파하는 필드 중에는 대표적으로 traceId와 spanId가 있다. 이런 필드들은 컨텍스트에 보관하는데, 여러 서비스를 거치더라도 일관성을 유지하고 싶은 필드가 더 있다면 컨텍스트에 추가해도 된다. 이렇게 별도로 추가하는 필드는 간단하게 "Baggage"라고 부른다.
...
> 현시점에는 baggage 항목의 개수나 크기에 제한을 두지 않았다. 하지만 baggage가 너무 많으면 시스템 처리량이 줄어들거나 RPC 지연 시간이 늘어날 수 있다는 점을 명심해두자. baggage가 극단적으로 많아지면 전송 계층<sup>transport-level</sup> 메시지나 헤더 용량을 초과하게 돼, 애플리케이션이 비정상 종료<sup>crash</sup>될 수 있다.

단순 이름 매핑 같이, 특별한 설정이 필요 없는 필드들은 프로퍼티를 사용해 정의할 수 있다:
...
트레이스<sup>trace</sup> ID와 마찬가지로 Baggage는 보통 헤더의 형태로 메시지 또는 요청에 추가된다. 태그는 span에 담아 Zipkin에 전송되는 키 값 쌍이다. 기본적으로 Baggage 값은 span에 추가하지 않으므로, 설정을 수정해주지 않으면 Baggage를 기준으로 span을 검색할 수 없다.
...
샘플링은 Zipkin과 같은 트레이싱<sup>tracing</sup> 백엔드에만 적용되는 개념이다. 참고로, 트레이스<sup>trace</sup> ID는 샘플링하는 비율에 관계없이 로그에 남는다. 샘플링은 모든 요청을 추적하는 대신에, 일부 요청만 지속적으로 추적해서 시스템 과부하를 방지한다.
...
기본적으로 샘플러는 리프레시 스코프 메커니즘과 함께 동작한다. 즉, 런타임에 샘플링 프로퍼티를 변경할 수 있고, 애플리케이션을 리프레시하면 변경 사항이 반영된다. 하지만 샘플러를 감싼 프록시를 생성하고서 샘플러를 너무 일찍 호출하면 (ex. `@PostConstruct` 애노테이션을 선언한 메소드 안에서 호출), 데드락이 발생할 수 있다. 이럴 땐 샘플러 빈을 생성하는 코드를 직접 작성하거나, `spring.sleuth.sampler.refresh.enabled` 프로퍼티를 `false`로 설정해 리프레시 스코프 지원을 비활성해라.
...
간편하게 빈을 주입받고 싶으면 `@HttpClientSampler`와 `@HttpServerSampler` 애노테이션을 사용하면 되고, 빈의 이름은 각 애노테이션에 정의된 스태틱 문자열 `NAME` 필드를 참조하면 된다.
...
Sleuth는 카프카나 JMS와 같은 메시지 처리 시스템을 계측하기 위한 `MessagingTracing` 빈을 자동으로 설정해준다.
...
간편하게 빈을 주입받고 싶으면 `@ProducerSampler`와 `@ConsumerSampler` 애노테이션을 사용하면 되고, 빈의 이름은 각 애노테이션에 정의된 스태틱 문자열 `NAME` 필드를 참조하면 된다.
...
보통은 Zipkin으로 span을 전송할 땐, span의 서비스 이름을 `spring.application.name` 프로퍼티 값과 동일하게 세팅하길 바랄 거다. 기본적으로 Sleuth도 그렇게 동작한다. 하지만 예외적인 상황도 존재한다. 때에 따라서는 애플리케이션에서 만드는 모든 span에 다른 서비스명을 지정해야 하는 경우가 있다. 이럴 땐 애플리케이션에 다음 프로퍼티를 지정해 서비스명을 재정의할 수 있다 (아래 예제는 서비스 이름을 `myService`로 정의한다):
...
먼저 Sleuth는, 주어진 이름의 패턴에 따라 span 리포트를 자동으로 건너뛸 수 있는 `SpanFilter` 빈을 등록한다. `spring.sleuth.span-filter.span-name-patterns-to-skip` 프로퍼티에는 기본으로 건너뛸 span의 이름 패턴이 담겨 있다. `spring.sleuth.span-filter.additional-span-name-patterns-to-skip` 프로퍼티는 기존 span 이름 패턴에 덧붙일 패턴을 추가한다. 이 기능을 비활성화고 싶다면 `spring.sleuth.span-filter.enabled`를 `false`로 설정하면 된다.
...
위 예제에선 trace ID가 `5e8eeec48b08e26882aba313eb08f0a4`인 것을 확인할 수 있다. 이 로그 설정은 Sleuth가 자동으로 세팅한다. `spring.sleuth.enabled=false` 프로퍼티를 이용해 Sleuth를 비활성화하거나, `logging.pattern.level` 프로퍼티를 직접 변경할 수 있다.
...
- 추가적인 appender 두 가지를 주석 처리했다 (console, standard 로그 파일) .
- 이전 섹션에서 보여준 예시와 동일한 로그 패턴을 사용한다.

> 커스텀 `logback-spring.xml`을 사용한다면, `application` 프로퍼티 파일이 아닌 `bootstrap`에 `spring.application.name`을 전달해야 한다. 그렇지 않으면 커스텀 logback 파일에서 프로퍼티를 제대로 읽어들이지 못한다.

---

## 4.7. Self Documenting Spans

A declarative format of representing span configuration was introduced via the `DocumentedSpan` abstraction. By analyzing Sleuth’s source code an appendix with all span characteristics is created (including allowed tag keys and event names). You can check the [Sleuth Spans appendix](https://docs.spring.io/spring-cloud-sleuth/docs/3.1.11/reference/htmlsingle/#sleuth-spans) for more information.

`DocumentedSpan` 인터페이스를 도입한 덕분에, span 설정은 보면 어떤 작업이 수행된 것인지 바로 파악할 수 있게 됐다. Sleuth의 소스 코드를 잘 보면 허용하는 태그 키와 이벤트 이름 등 모든 span의 특성들을 파악할 수 있다. 자세한 내용은 [Sleuth Span 부록](https://docs.spring.io/spring-cloud-sleuth/docs/3.1.11/reference/htmlsingle/#sleuth-spans)에서 확인할 수 있다.

---

## 4.8. Traces Actuator Endpoint

Spring Cloud Sleuth는 작업이 완료된 span을 저장할 수 있는 액추에이터 엔드포인트 `traces`를 함께 제공한다. 이 엔드포인트에 HTTP GET 메소드를 통해 질의하면 간단하게 저장된 span의 목록을 조회할 수 있고, HTTP POST 메소드를 통해 질의하면 목록을 조회하고 동시에 비울 수 있다.
...
Spring Cloud Sleuth의 핵심 기능들에 어느정도 익숙해졌다면, 계속해서 [Spring Cloud Sleuth의 통합 기능들](https://docs.spring.io/spring-cloud-sleuth/docs/3.1.11/reference/htmlsingle/#integrations.adoc)에 대해 읽어봐라.