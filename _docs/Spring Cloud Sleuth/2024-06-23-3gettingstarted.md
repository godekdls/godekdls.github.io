...
span은 시작하고 중단할 수 있으며, span은 시간 정보를 지속적으로 추적한다. span을 하나 생성했다면, 이후 어느 시점에는 반드시 종료해줘야 한다.
...
이번 섹션에선 Spring Cloud Sleuth의 핵심 기능 몇 가지를 알아볼 수 있는 간단한 "Hello World!" 웹 애플리케이션을 만드는 방법을 설명한다. 여기서 만드는 프로젝트는 대부분의 IDE가 지원하는 메이븐<sup>Maven</sup>을 통해 빌드한다. 트레이서<sup>tracer</sup> 구현체로는 [OpenZipkin Brave](https://github.com/openzipkin/brave)를 사용한다.
...
위 코드는 실제로 빌드되는 코드다. `mvn package` 명령어를 실행하면 테스트해볼 수 있다 (지금은 “jar will be empty - no content was marked for inclusion!”라는 경고가 보일텐데, 일단은 무시해도 된다).

> 원한다면 이 시점에서 프로젝트를 IDE로 임포트해도 좋다 (최신 Java IDE는 대부분 메이븐을 기본으로 지원한다). 단순하게 가기 위해 이 예제에서는 일반 텍스트 편집기를 계속 사용한다.

### 2.2.2. Adding Classpath Dependencies

필요한 의존성들을 넣어주려면 `pom.xml`을 열어 `parent` 섹션 바로 아래에 `spring-boot-starter-web` 의존성을 추가해라:
...
이 애플리케이션을 완성하려면 하나의 Java 파일을 만들어야 한다. 메이븐은 기본적으로  `src/main/java`에 있는 소스 코드를 컴파일하므로, 이대로 디렉토리를 생성한 다음, `src/main/java/Example.java`라는 이름의 파일을 추가하고 아래 코드를 채워넣어라:
...
과제를 해결해보기 앞서 조금 더 알아보고 싶다면, [Spring Cloud Sleuth 사용하기](../using-spring-cloud-sleuth)를 읽어봐라. 시간이 많지 않다면 [Spring Cloud Sleuth 기능](../features)으로 넘어가도 좋다.
