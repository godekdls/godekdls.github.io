...
배치 잡을 실행할 때 발생하는 주 이슈는 `Job`이 재시작할 때의 동작과 관련있다.
잡을 실행할 때 특정 `JobInstance`의 `JobExecution`가 이미 존재한다면, '재시작'으로 간주된다.
모든 잡이 중단된 지점부터 재시작할 수 있다면 이상적이겠지만, 불가능할 때가 있다.
이런 경우에는 개발자가 직접 새 `JobInstance`를 생성해야 한다.
물론 스프링 배치를 사용하면 문제가 조금 쉬워지는데,
절대 `Job`을 재실행해선 안되고 항상 새 `JobInstance`로 실행해야 한다면 restartable 프로퍼티 값을 'false'로 설정하면 된다:
...
위 코드에서 보여주는 바와 같이 재시작 불가능한 job의 `JobExecution`을 처음으로 생성할 때는 문제 없이 잘 생성된다.
그러나 두번째 시도부터 `JobRestartException`이 발생한다.

### 4.1.2. Intercepting Job Execution

`Job`을 실행 중일 때 잡의 라이프사이클 동안 발생한 이벤트들을 통지받아서 원하는 코드를 실행할 수도 있다.
`SimpleJob`은 적절한 타이밍에 `JobListener`를 호출함으로써 이를 지원한다.
...
job의 listeners 요소를 통해 `SimpleJob`에 `JobListeners`를 등록할 수 있다.
...
XML 네임 스페이스에서 선언되거나 `AbstractJob`의 서브 클래스를 사용한 job은 선택적으로 런타임에 사용할 job 파라미터 validator를 선언 할 수 있다.
job을 실행할 때 꼭 필요한 파라미터가 있을 때 검증하는 용도로 유용하다.
간단한 필수/옵션 파라미터를 검증하는 데 사용할 수 있는 `DefaultJobParametersValidator`를 지원하며, 좀 더 복잡한 제한 조건이 있다면 인터페이스를 직접 구현할 수도 있다.

자바 빌더를 통해 validator를 설정하면 된다:
...
이 경우 `@EnableBatchProcessing`이 배치잡 생성을 위한 기반 설정이 된다.
이 기반 설정 내에서 주입(autowired)받을 수 있는 여러 빈과 함께 `StepScope` 인스턴스가 생성된다:
...
디폴트 구현체는 위에서 언급된 빈을 제공하며 context를 제공하기 위한 `DataSource` 빈을 필요로 한다.
...
기본 설정만 하면 잡을 설정할 때 기본으로 제공되는 빌더 팩토리를 활용할 수 있다.
...
`@EnableBatchProcessing`을 사용하면 `JobRepository`를 제공한다.
이 섹션은 원하는 repository를 설정하는 방법을 설명한다.

위에서 설명한대로, `JobRepository`는 `JobExecution`나 `StepExecution`같은 
스프링 배치에서 사용되는 여러 도메인 오브젝트를 CRUD해오기 위한 용도로 사용된다.
이는 `JobLauncher`, `Job`, `Step`같은 주요 프레임워크 기능에 필요하다.
...
이는 실패 후 재시작할 때 꼭 필요한 state값을 가진 배치 메타 데이터가 올바르게 저장되는 것을 보장해야 하기 때문이다.
레포지토리 메소드에 트랜잭션 처리가 없다면 이를 보장할 수 없다.
두 프로세스가 동시에 같은 job을 실행하려고 하면 하나만 성공해야하므로 `create*` 메소드의 isolation 레벨은 별로 지정한다.
메소드의 디폴트 isloation 레벨은 SERIALIZABLE로, 극단적이라고 느껴질 수도 있다:

The default isolation level for that method is SERIALIZABLE, 
which is quite aggressive: READ_COMMITTED도 잘 동작한다;
두 프로세스가 충돌하는 경우만 아니면 READ_UNCOMMITTED도 괜찮다.
하지만 `create*` 메소드 호출이 매우 짧기 때문에 SERIALIZED이 딱히 문제되지는 않는다.(데이터베이스 플랫폼이 이를 지원 한다면.)
그래도 원하면 오버라이드할 수 있다: 
...
메타데이터 테이블의 프리픽스도 `JobRepository`의 속성을 변경해 바꿀 수 있다.
기본적으로는 BATCH_라는 프리픽스가 달린다. 예를 들어 BATCH_JOB_EXECUTION, BATCH_STEP_EXECUTION.
...
[JobExecution](https://godekdls.github.io/Spring%20Batch/domainlanguage/) 이 생성되고나면,
job의 메소드를 실행하기위해 넘겨져서, 마지막엔 caller에게 `JobExecution`를 리턴한다.
...
![Asynchronous Job Launcher Sequence](./../../images/springbatch/job-launcher-sequence-sync.png)![Job Launcher Sequence](./../../images/springbatch/job-launcher-sequence-async.png)
...
두 가지 모두 같은 context를 사용하거나 다른 context 여러 개를 사용할 수 있다.
...
Perl, Ruby같은 쉘 스크립트나 an, maven같은 '빌드 도구'가 아니어도 Java 프로세스를 시작하는 방법은 많다.
그러나 예제에서는 대부분이 익숙하다고 느낄 쉘 스크립트를 사용한다.

#### The CommandLineJobRunner

스크립트로 job을 실행하면 자바 Jirtual Machine을 실행하기 때문에 진입점으로 메인 메소드를 가진 클래스가 필요하다.
스프링 배치는 이를 위해 `CommandLineJobRunner`라는 구현체가 있다.
...
이 모든 건 넘겨받은 인자(argument)만 있으면 가능하다.
아래는 필수 인자들이다:
...
대부분 manifast를 jar 파일 안의 메인 클래스에 선언하고 싶겠지만, 단순화를 위해 클래스를 직접 사용한다.
...
이 말은 직접 실행하는 쉘 스크립트 같은 운영체제 프로세스만 다룬다는 뜻이다.
...
여기서 기억해둬야 할 점은 `ExitStatus`는 프레임워크에서 설정하는(혹은 개발자가 직접) 종료 코드라는 프로퍼티가 있으며,
...
`CommandLineJobRunner`는 `ApplicationContext`를 생성하는 클래스이므로 context 내에 함께 묶일 수 없고, 재구현한 값들을 주입해줄 필요가 있다.
...
배치 잡은 보통 긴 작업을 뜻하므로 job은 비동기로 실행해야 한다.
...
`Job`은 여전히 실행중인데도 논블로킹(nonblocking) 방식의 처리로 인해 컨트롤러는 즉시 응답할 수 있으며, `HttpRequest`를 사용한다면 이 방식이 필수다.
아래는 예제이다:
...
job이 여러 곳에서 생성되서(e.g. 자식 context) 어플리케이션 컨텍스트에서 job을 수집하고 싶을 때도 유용하게 쓰인다.
커스텀 `JobRegistry` 구현체 또한 등록된 job의 이름이나 프로퍼티를 관리할 때 유용할 수 있다.
기본으로 제공되는 구현체는 job 이름을 job 인스턴스에 매핑한 간단한 map 기반이다.

`@EnableBatchProcessing`를 선언했다면 `JobRegistry`를 기본으로 사용한다.
자체 구현체를 사용하려면:
...
빈 post processer를 사용하거나 register lifecycle 컴포넌트를 사용해서 `JobRegistry`를 등록할 수 있다.
...
위 예제에서 자식 context post-processor를 포함할 수 있도록 (e.g 상위 빈을 정의함으로써) 빈 id를 만들었지만, 필수는 아니다.

#### AutomaticJobRegistrar

lifecycle component는 자식 context를 생성하고 컨텍스트들에서 만든 job을 등록한다.
이렇게 해서 좋은 점은 자식 컨텍스트에 있는 job 이름은 registry 전역적으로 유일해야하는 건 동일하지만
의존성은 이름은 그렇지 않다는 것이다.
...
여러 모듈로 구성된 어플리케이션에 설정된 잡을 통합하기도 쉬워진다.
...
registrar에는 `ApplicationContextFactory` array와(여기선 편리한 팩토리 빈으로 생성했다) `JobLoader` 두 개의 필수 프로퍼티가 있다.
...
예를 들어 기본 상위 컨텍스트 및 하위 위치에 정의 된 작업이있는 경우이 방법이 바람직 할 수 있습니다.
...
그런데 이 두 오퍼레이션을 함께 사용하면 배치에서 흔히 쓰는 중단, 재시작, job 요약 등의 모니터링을 할 수 있다.
...
위의 오퍼레이션은 `JobLauncher`, `JobRepository`, `JobExplorer`, `JobRegistry`같은 다양한 인터페이스 메소드를 보여준다.
즉, `JobOperator`를 구현한 `SimpleJobOperator`는 의존성(dependency)이 많다.
...
> job repository에 테이블 프리픽스를 설정했다면 job exploerer에도 지정하는 걸 잊어선 안된다.
...
`JobLauncher`는 이전 파라미터 셋과 다른 값으로 새 `JobInstance`를 실행시킬 때 새로운 `JobParameters` 오브젝트가 필요한 반면,
`startNextInstance` 메소드는 `Job`과 연결된 `JobParametersIncrementer`를 통해 새 인스턴스를 만들도록 강제한다.
...
`JobParametersIncrementer`은 가지고 있는 `JobParameters`에서 필요한 값을 증가시켜 다음에 사용될 `JobParameters` 오브젝트를 리턴한다.
프레임워크에서는 `JobParameters`의 어떤 값을 바꿔야 '다음' 인스턴스라고 할 수 있는지 알 수 없기 때문에 유용한 전략이다.
...
그러나 반대의 경우에는 이전에 사용된 값을 넘겨 받아 바로 다음에 사용할 값을 리턴한다.
...
강제로 즉각 종료시키는 것은 불가능하기 때문에 바로 종료시키는 것은 아니다.
...
실패한 job은 재실행할 수 있다 (Job이 restartable로 설정된 경우에 한해서).
`ABANDONED` 상태인 job은 프레임워크에 의해 재실행되지 않는다.
job을 재실행할 때 step을 스킵하고 싶을 때도 `ABANDONED`를 사용한다:
이전에 실패한 job이 실행 중일 때 `ABANDONED`로 마킹된 step을 만나면 다음 step으로 넘어간다
(job flow 정의와 step execution의 종료 코드에 따라 달라진다).

프로세스가 죽으면 ("Kill -9" 명령어를 사용했거나 서버가 다운된 경우) 당연히 job은 실행 중이 아닌데도
`JobRepository`는 프로세스가 죽었다는 통지르 받을 수가 없어서 이를 알아채지 못한다.
execution이 실패하거나 중단되었다고 판단되면 이를 수동으로 알려줘야 하는데 (상태를 `FAILED`나 `ABANDONED`로 바꿔야 한다),
이는 비지니스 요구사항이기 때문에 자동화할 수는 없다.
재시작할수 없는 job이거나(restartable=false) 데이터가 유효하다는 걸 확신할 수 있을 때만 status를 `FAILED`로 바꿔라.
