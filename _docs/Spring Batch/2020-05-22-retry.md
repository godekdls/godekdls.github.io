...
상태가 없는(stateless) 재시도는 항상 실패했을 때와 동일한 스레드에서 retry 콜백을 실행한다.
...
어떤 작업이 실패했는 지는 각 재시도의 상태를 보고 알 수 있다.
...
더 이상 재시도하지 못한다면 `RetryCallback`를 호출하는 대신
...
처리도 담당한다.
간단한 구현체는 `RetryExhaustedException`를 던져 감싸고 있는 모든 트랜잭션을 롤백시킨다.
...
결정을 좀 더 세분화하려면 자체 retry policy를 구현해야 한다.
...
아래 코드는 `BackOffPolicy` 인터페이스 정의다:
...
매번 `RetryContext`, `RetryContext`과 함께 콜백받을 수 있다.

아래 코드는 `RetryListener` 인터페이스 정의다:
...
가장 간단하게는 모든 재시도 전후에 `open`, `close`에서 콜백받고
`onError`는 `RetryCallback`를 사용 할 매다다 호출한다.
