...
> [스프링 배치 공식 reference](https://docs.spring.io/spring-batch/docs/4.2.x/reference/html/index-single.html#repeat) 를 한글로 번역한 문서입니다.
...
그렇지 않다면 위 예제에서 처럼 콜백과 외부 policy에 결정을 위임하는 한
사실상 무한 루프나 다름 없다.
...
아래는 `ExceptionHandler` 인터페이스 정의다:
...
매번 `RepeatContext`, `RepeatStatus`과 함께 콜백받을 수 있다.

아래는 `RepeatListener` 인터페이스 정의다:
...
`before`, `after`, `onError`는 `RepeatCallback`을 사용 할 매다다 호출한다.

리스너가 둘 이상이라면 리스트 안에 저장하므로 순서가 정해져 있다.
이 때는 `open`, `before` 메소드는 동일한 순서로 호출하는 반면 
