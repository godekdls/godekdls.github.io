...
다른 고급 기술도 제공하는데, 최적화나 파티셔닝같은 기법을 사용하면 
...
시스템이 플랫(flat) 파일에 의존한다면 파일 백업 절차를 수립하고 문서화하고 정기적으로 테스트해야 한다. 
...
(무중단(running 24x7)으로 가동하려는 기업이 많을수록 배치 윈도우를 확보하기 힘들어진다는 점에 주의해라).
...
물리적 잠금을 사용한다면 잠재적인 교착상태를 피하기 위해 매우 조심히 다뤄야 한다. 
...
그 다음 *batch/extract application*의 각 인스턴스가 나눠진 각 레코드 셋을 처리한다.
