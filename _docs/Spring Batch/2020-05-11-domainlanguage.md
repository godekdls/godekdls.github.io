...
스프링 배치는 견고하고 유지보수 가능한 시스템에서 일반적으로 사용하는 레이어, 컴포넌트, 기술 서비스의 물리적 구현체를 제공하는데,
...
아래 예시처럼, 자바 기반으로 설정하는 경우에 제공되는 여러 builder로 `Job`의 초기화할 수 있다.
...
1월 1일 실행한 배치가 실패해서 다음날 다시 실행되어야 하는 경우, 1월 1일의 작업과 동일한 작업을 재실행해야 한다. 
(보통 처리 날짜와 처리할 데이터가 일치하는데, 1월 1일에 실행하면 1월 1일의 데이터를 처리한다는 뜻이다)
따라서 각 `JobInstance`는 여러 개의 실행 결과를 가질 수 있고(`JobExecution`은 이 챕터 뒷 부분에 자세히 나온다),
...
예를 들어, 앞에서 나온 EndOfDay 케이스에서는, 데이터가 실행되어야 하는 날짜를 의미하는 컬럼이 있을 것이다.
...
`JobExecution`은 Job 실행을 한번 시도했다는 기술적인 개념이다.
...
job이 실패하고, 밤새도록 문제를 찾느라 'batch window'가 이제야 닫혔다고 가정해보자.
또한 window는 9:00 PM에 시작하고, 이 job은 다음날 01-01 데이터 처리를 중단 되었던 위치에서부터 다시 시작해서 9:30에 성공적으로 종료됐다고 가정해보자.
...
`StepExecution`은 `JobExecution`과 유사하게 `Step`을 실행할 때마다 생성된다.
하지만 이전 단계 step이 실패해서 step이 실행되지 않은 경우는 execution을 저장하지 않는다.
`Step`이 실제로 시작됐을 때만 `StepExecution`을 생성한다.

`Step` execution은 `StepExecution` 클래스의 오브젝트로 표현된다.
...
아래 테이블은 `StepExecution`의 프로퍼티 리스트를 담고 있다.
...
|processSkipCount|프로세스에 실패해서 스킵된 횟수.|
...
`StepExecution` 오브젝트 또는 `JobExecution` 오브젝트에 속하는 상태(state)를 저장한다.
 ...
 예를 들어 flat한 파일을 인풋으로 받아 각 라인별로 처리한다면 프레임워크는 커밋 지점에 주기적으로 `ExecutionContext`를 저장한다.
 이렇게 하면 `ItemReader`는 실행 중 전원이 나가버리는 등의 치명적인 에러가 발생했을 때에도 state를 저장해둘 수 있다. 
...
이 값은 프레임워크가 각 커밋 전 업데이트하며, `ExecutionContext` 내 엔터디에 해당하는 여러 row를 포함 할 수 있다.
커밋 전에 통지를 받으려면 여러 `StepListener` 구현체(또는 `ItemStream`) 중 하나가 필요한데, 자세한 내용은 이 가이드 뒷부분에 나온다.
이전 예시와 동일하게 다음 날 `Job`이 재실행되었다고 가정한다.
재시작될 때 데이터베이스로부터 마지막 실행을 가리키는 `ExecutionContext`의 값들을 조회한다.
아래 예제처럼, `ItemReader`가 열릴 때 컨텍스트에 저장된 상태가 있는지 확인하고, 있다면 해당 컨텍스트를 참조해서 초기화시킨다.
...
위 코드가 실행되고나면 현재 라인은 40,322이 돼서, 중단됐던 위치부터 `Step`을 다시 실행할 수 있다.
실행 자체에 관해 기록한 통계 데이터도 `ExecutionContext`를 활용하면 된다. 
예를 들어, 여러 줄을 한 번에 처리해야하고 그 처리에도 순서가 있다면 몇번째 순서까지 진행됐는지도 기록해야한다.
(단순히 읽어온 라인 수를 기록하는 것과는 다르다)
따라서 `Step`이 끝날 때 몇 번째 순서까지 진행이 되었는지를 메일로 보낸다고 해보자.
...
`JobLauncher`는 아래 코드에서 처럼 주어진 `JobParameters` set으로 `Job`을 실행시키는 심플한 인터페이스다.
...
`JobRepository`로부터 유효한 `JobExecution`을 얻어와서 `Job`을 실행시키는 기능을 구현한다.

## 3.6. Item Reader

`ItemReader`는 `Step`에서 한 번에 하나의 item씩 읽어오는 작업을 추상화한 개념이다.
더 이상 읽어올 item이 없으면 `ItemReader`는 null을 리턴한다.
...
`ItemWriter`는 `Step`에서 한 번에 배치나 청크 단위로 item을 읽어오는 작업을 추상화한다.
보통 `ItemWriter`는 다음에 받을 input이 무엇인지는 알지 못하며 현재 받은 아이템만 알고 있다.
