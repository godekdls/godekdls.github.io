...
> [스프링 배치 공식 레퍼런스](https://docs.spring.io/spring-batch/docs/4.2.x/reference/html/index-single.html#scalability)를 한글로 번역한 문서입니다.
>
...
상태를 쓰레드별로 분리하지 않으면 이 컴포넌트를 멀티 쓰레드 `Step`에서 사용할 수 없다.
...
미들웨어는 내구성이 있어야 하며, 각 메세지를 컨슈머 하나에 전달한다는 걸 보장해야 한다.
