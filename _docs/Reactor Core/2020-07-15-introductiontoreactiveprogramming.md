...
- **Hot** 시퀀스는 각 `Subscriber`마다 처음부터 만들지 않는다. 오히려 나중에 구독한 구독자는 구독 *이후* 생산한 신호를 받는다. 단, 일부 hot 리액티브 스트림은 생산 데이터 전체 혹은 일부를 캐시해 두거나 이전 데이터를 재사용할 수 있다. 일반적인 관점으로 보면, hot 시퀀스는 구독자가 없을 때도 발생할 수 있다 (이 때는 예외적으로 "구독하기 전에 아무 일도 일어나지 않는다"는 규칙이 적용되지 않는다).
