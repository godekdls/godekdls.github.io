...
리액터 `Processor`를 직접 사용하는 것보다 `sink()`를 **한 번** 호출해서 `Processor`의 `Sink`를 획득하는 게 더 좋은 방법이다.
...
초기에 구독자가 없을 때에도 설정한 `bufferSize`까지 데이터 푸쉬를 허용한다. 이후에도 데이터를 소비해가는 `Subsriber`가 없다면 프로세서가 비워질 때까지 `onNext` 블록을 호출한다 (이때까지만 동시에 가능하다).
