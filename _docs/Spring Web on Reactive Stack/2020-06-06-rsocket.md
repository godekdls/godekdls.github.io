...
originalRefLink: https://docs.spring.io/spring/docs/current/spring-framework-reference/web-reactive.html#rsocket
---
...
위 코드에선 디폴트로 `@Controller`에서 클라이언트 responder를 찾는다. `@RSocketClientResponder` 등의 커스텀 애노테이션을 사용하고 싶다면 `RSocketMessageHandler`의 `setHandlerPredicate`로  전략을 바꿔야 한다. 어플리케이션 하나에서 클라이언트와 서버를 같이 사용하거나, 여러 클라이언트를 사용한다면 이 전략이 필요하다.
